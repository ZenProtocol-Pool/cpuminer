...
json_t *request_template(CURL *curl, const char *url, int *curl_err)
{
...
bool fulltest(const uint32_t *hash, const uint32_t *target)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if ( swab32(hash[1]) == swab32(target[1]) ) {
			applog(LOG_NOTICE, "We found a close match!");
		}
	}

	if (rc && opt_debug || rc && opt_hashdebug) {
...
bool stratum_authorize(struct stratum_ctx *sctx, const char *user, const char *pass)
{
    ...
    applog(LOG_NOTICE, "Logging in to pool...");
	if (!stratum_send_line(sctx, s))
		goto out;
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	...
	hex2bin(&sctx->job.header, header, 100);
