...
#define DEF_RPC_URL		"stratum+tcp://mine-us-west.leafpool.com:8800"
...
static enum sha256_algos opt_algo = ALGO_KECCAK;
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (have_stratum) {
		...
		sprintf(s,
			"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"00000000\", \"%s\"], \"id\":4}",
...
static bool get_work(struct thr_info *thr, struct work *work)
{
	...
	if (opt_benchmark) {
		memset(work->data, 0x55, 96);
		work->data[17] = swab32(time(NULL));
		memset(work->data + 24, 0x00, 32);
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	//for (i = 0; i < 4 && !++sctx->job.xnonce2[i]; i++);

	/* Assemble block header */
	//memset(work->data, 0, 128);
    //hex2bin( &work->data, sctx->job.header, 100);

    memcpy(&work->data, &sctx->job.header, 100);

    if ( ++work->nonce_extra >= 0xffffffff) work->nonce_extra = 0;
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		if (have_stratum) {
			...
			//if (work.data[24] >= end_nonce) //Used up all nonce?
		...
		if (memcmp(work.data, g_work.data, 100)) {
			memcpy(&work, &g_work, sizeof(struct work));
			work.data[24] = 0xffffffffU / opt_n_threads * thr_id;
		} else
			work.data[24]++;
		pthread_mutex_unlock(&g_work_lock);
		...
		if (max64 <= 0)
			max64 = 0x1fffffLL; //TODO check this
		if (work.data[24] + max64 > end_nonce)
			max_nonce = end_nonce;
		else
			max_nonce = work.data[24] + max64;
		...
		rc = scanhash_keccak(thr_id, work.data, work.target, max_nonce, &hashes_done);
	...
	while (1) {
		...
		if (stratum.job.job_id &&
			...
			if (stratum.job.clean) {
                if ( opt_debug )
                    applog(LOG_INFO, "Stratum sent a new block to mine");
				restart_threads();
			}
		}
...
int main(int argc, char *argv[])
{
	...
	rpc_user = strdup("zen1q6e7qpep0675vsnq29qyld5agwdkryusv9h9y9c3e3mq5qwcmrpzs0dxczd");
	rpc_pass = strdup("");
	...
	applog(LOG_INFO, "ZenProtocol CPU Miner v0.01");
    applog(LOG_INFO, "CryptominerOS - https://www.leafpool.com/");
